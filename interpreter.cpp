    ...
    try
    {
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                break;
                case OP_MORTGAGE:
                case OP_MORTGAGEVERIFY:
                {
                    if (stack.size() < 2)
                        return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);

                    valtype& backhash160 = stacktop(-2);
                    valtype& locktime = stacktop(-1);
                    bool fSuccess = true;
                    popstack(stack);
                    popstack(stack);
                    stack.push_back(fSuccess ? vchTrue : vchFalse);
                    if (opcode == OP_MORTGAGEVERIFY)
                    {
                        if (fSuccess)
                            popstack(stack);
                        else
                            return set_error(serror, SCRIPT_ERR_MORTGAGEVERIFY);
                    }
                }
                break;
