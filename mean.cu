// Cuckoo Cycle, a memory-hard proof-of-work by John Tromp // Copyright (c) 2018 Jiri Vadura (photon) and John Tromp
...
extern bool HashLessThan(const CBlock &block, const uint256 &target);
extern void FillBlockSolution(CBlock &block, uint32_t *solutions);
...
struct solver_ctx {
  ...
  solver_ctx(const trimparams tp) : trimmer(tp) {
                               ...
                               )
{
  ...
  for (u32 r = 0; r < range; r++) {
    ...
    for (unsigned s = 0; s < nsols; s++) {
      //print_log("Solution(%jx)", (uintmax_t)(nonce+r));
      ...
      if (pow_rc == POW_OK) {
        ...
        FillBlockSolution(block, prf);

        if(HashLessThan(block, pTarget)) {
          return 1;
        } else {
          //printf("Self verify failed due to high hash\n");
        }
...
CALL_CONVENTION SolverCtx* create_solver_ctx(SolverParams* params) {
  ...
  SolverCtx* ctx = new SolverCtx(tp);
...
int cuckoo_mean(unsigned char *base64, int length, const uint256 &ptarget, int device, int &good_sols, CBlock &block)
{
  ...
  print_log("Looking for %d-cycle on cuckoo%d(\"%s\",%llu", PROOFSIZE, NODEBITS, header, nonce);
  if (range > 1)
    print_log("-%llu", nonce+range-1);
  print_log(") with 50%% edges, %d*%d buckets, %d trims, and %d thread blocks.\n", NX, NY, params.ntrims, NX);*/

  static SolverCtx* ctx[16] = { 0 };
  if(!ctx[device]) ctx[device] = create_solver_ctx(&params);
  ...
  int ret = run_solver(ctx[device], header, sizeof(header), nonce, range, ptarget, good_sols, block);
